...
class links extends masterclass{
	...
	function items(){
		...
		$this->sql['items'] = "SELECT l.*, GROUP_CONCAT(t.tag) as tags FROM links l ";
		
		if($tag!=''){
			$this->sql['items'] .= "  JOIN linktags t ON t.link_id = l.id AND t.tag='$tag'"; 
			$this->sql['count'] .= "  JOIN linktags t ON t.link_id = l.id AND t.tag='$tag'";
		}else
			$this->sql['items'] .=  " LEFT JOIN linktags t ON l.id=t.link_id";
